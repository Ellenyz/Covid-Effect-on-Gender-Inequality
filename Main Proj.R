...
WH$Relative_wh <- WH$AHRSWORKT/WH$UHRSWORKT
...
plt3 <- ggplot(data=Relative_WH_agg, aes(x=MONTH, y=avg_Relative_wh, group=factor(SEX), shape=factor(SEX), colour=factor(SEX))) + 
  ggtitle("Average Relative Working Hours") + 
  xlab("Month") + ylab("Working Hour Growth") + geom_line()+ geom_point() + geom_smooth(method='lm', linetype=4, size=0.5, se=FALSE) 
